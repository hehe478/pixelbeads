          ...
          <div className="bg-white dark:bg-[#1a1a2e] rounded-2xl p-1.5 shadow-sm border border-gray-100 dark:border-gray-700 flex gap-2">
             <div className="flex-1 min-w-0">
               ...
               <select 
                 ...
                 className="w-full bg-transparent border-none text-sm font-bold text-gray-800 dark:text-white p-0 pl-2 focus:ring-0 cursor-pointer truncate"
               >
                 {paletteConfig.brand === '自定义' && <option value="自定义">自定义品牌</option>}
             ...
             <div className="flex-1 min-w-0">
               ...
               <select 
                 value={paletteConfig.set === 'custom' ? (paletteConfig.activeCustomId || '') : paletteConfig.set}
                 onChange={(e) => {
                     const val = e.target.value;
                     const isCustom = customPalettes.some(p => p.id === val);
                     if (isCustom) selectCustomPalette(val);
                     else setSet(Number(val));
                 }}
                 className="w-full bg-transparent border-none text-sm font-bold text-gray-800 dark:text-white p-0 pl-2 focus:ring-0 cursor-pointer truncate"
               >
                 {customPalettes.length > 0 && (
                    <optgroup label="我的配色库">
                        {customPalettes.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                    </optgroup>
                 )}
                 {availableSets.length > 0 && (
                    <optgroup label="官方套装">
                        {availableSets.map(s => <option key={s} value={s}>{s}色套装</option>)}
                    </optgroup>
                 )}
               </select>
             </div>
             <button onClick={() => setIsManagerOpen(true)} className="w-10 h-10 rounded-xl bg-primary/10 text-primary flex items-center justify-center hover:bg-primary hover:text-white transition-colors shrink-0">
        ...
        <div className="px-4 pt-4 pb-2">
          <div className="relative w-full h-48 rounded-xl overflow-hidden group shadow-lg cursor-pointer" onClick={() => navigate('/editor/featured')}>
            ...
            <div className="absolute bottom-0 left-0 p-5 w-full">
              <div className="flex items-center gap-2 mb-1"><span className="px-2 py-0.5 rounded-md bg-primary text-white text-xs font-bold uppercase tracking-wider">热门推荐</span><span className="text-white/80 text-xs font-medium">每周更新</span></div>
